...
| Native ARM64 real-device comparison | M0A complete: owner verified the Asteria process as native ARM64 on a Surface Pro 11th Edition (Snapdragon X Plus, 16 GB RAM), reported repeated 2560×1440 approximately 60 FPS Apollo AV1 parity with official x64 Moonlight under emulation, and subjectively smoother Asteria menus/settings |
| Local compile | Not run: Qt/MSVC absent |
| Clean-machine portable launch | No separate clean-machine evidence recorded |
| Apollo lifecycle, H.264 1080p60 SDR, audio/input/gamepad | Per-case results and host version not recorded; first-preview release checks, not M0A blockers |
| Sunshine qualification | Not required for this owner-operated Apollo project/preview |
